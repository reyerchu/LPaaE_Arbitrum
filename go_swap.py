# Written by ReyerChu@deFintek.io by 2024/2/1
# platform == 1:Linux, 2:Mac, 3:Webhook server(flask)
platform = 1

config_file = ''
if (platform == 3):
    # webhook server
    config_file = '/var/www/defintek.io/webhook/config.ini'
else:
...
def get_config():
    ...
    global line_apikey, line_notification_headers
    global infura_url #, subgraph_URL
    ...
    # alert settings
    if (platform == 2):
        ALERT_PATH = config['ALERT']['alert_pathMac'] # Mac
    else:
        ALERT_PATH = config['ALERT']['alert_path'] # Linux
    # telegram settings
    ...
    # get API keys for Line and Telegram
    # init Line
    line_apikey = config['API KEY']['line']
    line_notification_headers = {
        "Authorization": "Bearer " + line_apikey,
    ...
    #subgraph_URL = config['URL']['subgraph_arbitrum'] #Arbitrum https://github.com/Uniswap/v3-subgraph/issues/65 
    # 設定代幣及pool地址
    ETH_address = config['ADDRESS']['eth_address']  # ETH 的地址通常表示为 0x0
    # WETH token contract
...
def send_message(msg):
    ...
    '''
    # another way for TG
    #tg_chat_id = 550417061
    #id = "ReyerChiaStatusBot" #get from tg_bot.getMe()
    #ret_tg = tg_bot.sendMessage(tg_chat_id, msg, parse_mode='HTML')
    #print("Sent alert to TG with return code:", ret_tg)
    '''
    # 3. send message to Line
    #ret_line = requests.post("https://notify-api.line.me/api/notify", headers=line_notification_headers, params={"Message": msg})
    #print("Sent alert to Line with satus code:", ret_line.status_code)
    return
...
def swap_usdc2weth(usdc_amount_in_human, need_balance_check: Optional [bool] = False) -> HexBytes:
    if (need_balance_check and check_balance_usdc(usdc_amount_in_human) == False): 
        # 要檢查錢包內 USDC => USDC 不夠
        print(f"Reyer Error swap_usdc2weth(): USDC in wallet < needed {usdc_amount_in_human}!")
        return 0
    # 已检查過是否已授权足够的 USDC
    # swapping...
...
def swap_weth2usdc(weth_amount_in_human, need_balance_check: Optional [bool] = False) -> HexBytes:
    if (need_balance_check and check_balance_weth(weth_amount_in_human) == False): 
        # 要檢查錢包內 WETH => WTH 不夠
        print(f"Reyer Error swap_weth2usdc(): WETH in wallet < needed {weth_amount_in_human}!")
        return 0
    # 已检查過是否已授权足够的 WETH
    # swapping...
...
def swap_usdc2ETH(usdc_amount_in_human, need_balance_check: Optional [bool] = False) -> HexBytes:
    if (need_balance_check and check_balance_usdc(usdc_amount_in_human) == False): 
        # 要檢查錢包內 USDC => USDC 不夠
        print(f"Reyer Error swap_usdc2ETH(): USDC in wallet < needed {usdc_amount_in_human}!")
        return 0
    # 已检查過是否已授权足够的 USDC
    # swapping...
...
def swap_ETH2usdc(eth_amount_in_human, need_balance_check: Optional [bool] = False) -> HexBytes:
    if (need_balance_check and check_balance_ETH(eth_amount_in_human) == False): 
        # 要檢查錢包內 ETH => ETH 不夠
        print(f"Reyer Error swap_ETH2usdc(): ETH in wallet < needed {eth_amount_in_human}!")
        return 0
    # swapping...
...
def wrap_ETH(eth_amount_in_human, need_balance_check: Optional [bool] = False) -> HexBytes:
    if (need_balance_check and check_balance_ETH(eth_amount_in_human) == False): 
        # 要檢查錢包內 ETH => ETH 不夠
        print(f"Reyer Error wrap_ETH(): ETH in wallet < needed {eth_amount_in_human}!")
        return 0
...
def add_LP(weth_amount_in_wei, usdc_amount_in_wei, adjusted_min_tick, adjusted_max_tick, need_balance_check: Optional [bool] = False) -> HexBytes:
    ...
    if (need_balance_check and check_balance_weth(weth_amount_in_human) == False): 
        # 要檢查錢包內 WETH => WTH 不夠
        print(f"Reyer Error add_LP(): WETH in wallet < needed {weth_amount_in_human}!")
        return 0
    usdc_amount_in_human = usdc_in_wei_to_human(usdc_amount_in_wei)
    if (need_balance_check and check_balance_usdc(usdc_amount_in_human) == False): 
        # 要檢查錢包內 USDC => USDC 不夠
        print(f"Reyer Error add_LP(): USDC in wallet < needed {usdc_amount_in_human}!")
        return 0
...
def make_CDP(needed_usdc): 
    ...
    if (needed_debt_in_wei <= 0): # 已借足夠, 無需動作 (Reyer??? 不確定是否要處理)
        return 0
    ...
    if (needed_collateral_in_human > 20): # 債資比 > AAVE_LTV (0.7)
        ...
        if (check_balance_usdc(needed_collateral_in_human) == False):
            ...
            if (check_balance_usdc(needed_collateral_in_human) == False): # 換完再 check 一次
                ...
                print(f"Reyer Error: USDC in wallet < needed {needed_collateral_in_human} USDC!")
                return 0
...
def check_and_adjust_balance(want_weth, at_least_usdc) -> bool: # True:All ok, False:Fail => Stop
    ...
    if (balance_value < want_value):
        ...
        print("Reyer Error: token value in wallet {:.3f} < want {:.3f}!".format(balance_value, want_value))
...
def go_signal(signal): # return 0:success 1:fail
    ...
    if (signal > 0): # 0. 已持有多單 => 要調整: 先清空 LP, 再 add_LP
        ...
        if (LP_position < 0):
            ...
            LP_position = 0
        """
    if (signal > 0 and LP_position <= 0): # 0. 已持有多單 => 不動作
        # 1. 已持有空單 => 清空單
        if (LP_position < 0):
            close_LP_all() # to be some WETH and some USDC
            close_CDP() # repay WETH and withdraw USDC
        ...
        #wrap_all_ETH() # Reyer: 錢包中的 ETH 不要變動???
        ...
        if (LP_position > 0):
            close_LP_all() # to be some WETH and some USDC
            close_CDP() # repay WETH and withdraw USDC (Reyer:清多單時應該不需要)
            LP_position = 0
        elif (LP_position < 0):
            close_LP_all() # to be some WETH and some USDC
            #close_CDP() # repay WETH and withdraw USDC (Reyer:清多單時應該不需要)
            LP_position = -2 # 調整到一半
        """
    elif (signal < 0 and LP_position >= 0): # 0. 已持有空單 => 不動作
        # 1. 已持有多單 => 清多單
        if (LP_position > 0):
            close_LP_all() # to be some WETH and some USDC
            close_CDP() # repay WETH and withdraw USDC (Reyer:清多單時應該不需要)
...
def handle_alert(alert_string):
    ...
    #alert_msg += "Symbol: {}\n".format(Alert_Symbol)
    #alert_msg += "Strategy: {}\n".format(Alert_Strategy)
    #alert_msg += "Price: {:.2f}\n".format(Alert_Price)
    #alert_msg += "StopLoss: {:.2f}\n".format(Alert_StopLoss)
    #alert_msg += "TakeProfit: {:.2f}\n".format(Alert_TakeProfit)
    #alert_msg += "Time: {}\n".format(Alert_Time)
    if (Alert_StopLoss):
    ...
    # 2. handling Alert
    if ("Long by Reyer26738832" in Alert_Signature):
        if go_signal(1) == 0:
            alert_msg += "Long\n"
    elif ("Short by Reyer26738832" in Alert_Signature):
        if go_signal(-1) == 0:
            alert_msg += "Short\n"
    elif ("Flat by Reyer26738832" in Alert_Signature):
        if go_signal(0) == 0:
            alert_msg += "Flat\n"
    else:
    ...
    status_msg = get_status() # get wallet balance and price info.
    status_msg += update_LP_position_and_uniswap_positions() # update LP_position, uniswap_positions
...
'''
# Initialize the Uniswap
#uniswap_py = Uniswap(address=wallet_address, private_key=wallet_private_key, version=3, web3=web3)
uniswap_defi = fetch_deployment(
    web3=web3,
...
# start to add testing code here ---------------
''' Sample: positions(1019872)
nonce        uint96 : 0
operator    address : 0x0000000000000000000000000000000000000000
token0      address : 0x82aF49447D8a07e3bd95BD0d56f35241523fBab1
...
if number_of_positions > 0:
    for idx in range(number_of_positions):
        ...
        # check if any alive LP
        nft_token_id = uniswap_positions[idx]
        nonce, operator, token0, token1, fee, tickLower, tickUpper, uniswap_newest_LP.liquidity, feeGrowthInside0LastX128, feeGrowthInside1LastX128, tokensOwed0, tokensOwed1, *_ = uniswapv3_posnft_contract.functions.positions(nft_token_id).call()
        print(f"{nonce}, {operator}, {token0}, {token1}, {fee}, {tickLower}, {tickUpper}, {uniswap_newest_LP.liquidity}, {feeGrowthInside0LastX128}, {feeGrowthInside1LastX128}, {tokensOwed0}, {tokensOwed1}\n")
exit()
'''
#LT_position = -1
#ST_position = LP_position
#sell_tx_hash = swap_weth2usdc(Decimal(0.1))
#sell_tx_hash = swap_usdc2weth(300)
#sell_tx_hash = swap_usdc2weth_output(0.1)
'''
# test Long ------------------------
go_signal(1)
# update LP_position, uniswap_positions and show status
status_msg = get_status() # get wallet balance and price info.
status_msg += update_LP_position_and_uniswap_positions() # update LP_position, uniswap_positions
send_message(status_msg)
# end of test ----------------------
'''
#exit()
# end to add testing code here ---------------



# update and show status
msg = get_status() # get wallet balance and price info.
msg += update_LP_position_and_uniswap_positions() # update LP_position, uniswap_positions
send_message(msg)


if not(platform == 3): # not webhook server
    msg = ""
    #print("flaskapp.py is running... not server!")
    ...
    # 以下是在 flask webhook 執行時打開
    app = Flask(__name__)

    # 设置日志记录
    #handler = RotatingFileHandler('flaskapp.log', maxBytes=10000, backupCount=3)
    ...
    @app.route('/')
    def home():
        return "Welcome to webhook for TradingView!"
    ...
    # 检查请求的内容类型
        ...
        if 'application/json' in content_type:
            data = request.json
            #app.logger.info('Received webhook: %s', data) # logging
            print("Received JSON data:", data)
        elif 'text/plain' in content_type:
            data = request.data.decode('utf-8')
            print("Received text data:", data)
            handle_alert("{}".format(data))
        else:
            print("Unsupported Media Type", content_type)
...
'''
# 測試
usdc_amount_to_swap_in_human = 2200
estimated_eth_got = Decimal(uniswap_py.get_price_input(usdc_checksum_address, ETH_address, usdc_in_human_to_wei(usdc_amount_to_swap_in_human), fee=FEE_TIER)) / (10 ** weth_decimals)
estimated_eth_price = usdc_amount_to_swap_in_human / estimated_eth_got
cprint(f"Estimated price: {estimated_eth_price}")
estimated_eth_price = uniswap_py.get_price_input(ETH_address, usdc_checksum_address, eth_in_human_to_wei(1), fee=FEE_TIER) / (10 ** usdc_decimals)
print(f"Estimated price: {estimated_eth_price}")
